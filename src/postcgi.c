...
#define PUT 1
#define POST 2
#define UNAUTHORISED "401 UNAUTHORISED"
...
int http_authentication(char* user_name, char* pass_word)
{
    ...
    sprintf(qry,"select * from users where user = '%s'",user_name);
    /* send SQL query */
    if (mysql_query(conn, qry))
    {
        fprintf(stderr, "%s\n", mysql_error(conn));
	...
	else 
	{
		...
		else
		{
			memset(msg,'\0',MAX_RCV_MSG_SIZE);
		  	if (NULL == fgets(msg,MAX_RCV_MSG_SIZE, stdin))
			{
		 		printf("<P>Error in invocation - unknown content length</P>");
				perror("<P>error<P>");	
			}
			...
			if ((parse_user_pwd(msg,username,passwd,&input)== SUCCESS))
			{
				...
				{	
					...
					if (NULL != f)
					{
						...
						fgets(temp,1000,f);
		  				fclose(f);
						printf("<P>201 Created</P>");
						printf("<P>YOUR DATA HAS BEEN SUBMITED AND THE URL IS %s</P>",script+strlen(temp)+4);
						fflush(stdout);
						exit(SUCCESS);
					}
					f = fopen("./conf","r");
					...
					{	
						...
						else
						{
							...
							fgets(temp,1000,f);
		  					fclose(f);
							printf("<P>201 Created</P>");
							printf("<P>YOUR DATA HAS BEEN SUBMITED AND THE URL IS %s</P>",root_path+strlen(temp)+3);
							fflush(stdout);
						}
						exit(SUCCESS);
					}	
					...
					//printf("its a put method");
						...
						if (strstr(script,"php") == NULL)
						{
							...
							else
							{
								...
								f = fopen("./conf","r");
								fgets(root_path,1000,f);
								printf("<P>%s</P>",CREATED);
								printf("<P>YOUR DATA  HAS BEEN SUBMITED AND THE URL IS /%s</P>",temp + strlen(root_path)+4);
								fflush(stdout);
								fclose(f);
								exit(FAILURE);
							}
				...
				{
					printf("<P>%s</P>",UNAUTHORISED);
					printf("<P>YOU ARE NOT THE AUTHORIZED USER TO USE PUT REQUESTS please provide user name and passwd</P>");
			...
			{	
				printf("<P>username or password not provided</P>");
				printf("<P>%s</P>",UNAUTHORISED);
				printf("<P>YOU ARE NOT THE AUTHORIZED USER TO USE PUT REQUESTS please provide user name and passwd</P>");
				printf("<P>USAGE --user=username --password=password</P>");
			}
